...
def main(log):
	...
	if args.interactive:
		try:
			...
			sys.exit(1)
		interactive.interactive_menu(log)
		sys.exit(0)
	db = Database()
	if args.f:
		fname = args.f
	...
	if args.g:
		...
		logData = 'Generated %s random records'%(str(args.g))
		log.write(str(time.now())+logData+'\n')
		print logData
		logData = 'Time taken for creation %2.5s s ' % db.creattime
		print logData
		log.write(str(time.now())+logData+'\n')
	else:
		db.create(fname)
	...
	if args.s :
		p = db.select(args.s)
		db.printdb(p)
		logData = 'Time taken for select %2.5s s '% db.seltime
		print logData
		log.write(str(time.now())+logData+'\n')
		logData = 'Time taken for print %2.5s s' % db.printtime
		print logData
		log.write(str(time.now())+logData+'\n')
	if args.i:
		db.insert(fname,args.i)
		logData = 'Time taken for insert %2.5s' % db.instime
		print logData
		log.write(str(time.now())+logData+'\n')
	if args.d:
		db.delete(fname,args.d)
		logData = 'Time taken for delete %2.5s' % db.deltime
		print logData
		log.write(str(time.now())+logData+'\n')
	if args.u:
		db.update(args.u,fname)
		logData = 'Time taken for update %2.5s' % db.updtime
		print logData
		log.write(str(time.now())+logData+'\n')
